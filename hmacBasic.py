...
# sign message
def send_hmac(*arg): # send_hmac(url, json_data)
	...
	if len(arg) > 3:
		...
		# the blob we use here is the path of the request that we want to make, the json data, and the clients timestamp
		digester.update(arg[1]+str(arg[3])+str(timenow))
	...
	# sign url message
		...
		digester = hmac.new(secret)
        	digester.update(arg[2]+str(timenow))
        	digest = digester.hexdigest()
...
def authorize_hmac(app):
    	# the shared private key that is used between api and client, this implementation doesn't use public keys to identify users since there is only one user...
	...
	if request.args.get('hash') and request.args.get('timestamp'):
		...
		if time.time() < float(client_timestamp)+float(600):
			...
			if request.json:
				digester = hmac.new(secret)
				# the blob we use here is the path of the request that was made, the json data, and the clients timestamp
				digester.update(request.path+str(request.json)+str(client_timestamp))
				digest = digester.hexdigest()
			...
			# verify url var message
				...
				digester = hmac.new(secret)
				# since all the message payload is in the url we only need to blob the request path and client timestamp
				digester.update(request.path+str(client_timestamp))
				digest = digester.hexdigest()
				return digest == client_hash
		else:
			return False
